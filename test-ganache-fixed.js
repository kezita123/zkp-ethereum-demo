...
async function main() {
    ...
    // Create a VALID bytes32 commitment (exactly 32 bytes)
    console.log("2️⃣ Generating ZK proof commitment...");
    ...
    // Try to store the same commitment again (should fail)
    console.log("5️⃣ Trying to store duplicate commitment (should fail)...");
    try {
        await contract.storeProof(commitment);
        console.log("   ❌ This should have failed!");
    } catch (error) {
        console.log(`   ✅ Correctly failed: "Proof already exists"\n`);
    }
